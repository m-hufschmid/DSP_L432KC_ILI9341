...
void testFastLines(uint16_t color1, uint16_t color2) {
	...
	Displ_CLS(BLACK);
	for (y = 0; y < h; y += 5)
		Displ_Line(0, y, w - 1, y, color1);
	for (x = 0; x < w; x += 5)
		Displ_Line(x, 0, x, h - 1, color2);
}

void testRects(uint16_t color) {
	int n, i, i2, cx = _width / 2, cy = _height / 2;
...
void testFilledRects(uint16_t color1, uint16_t color2) {
	int n, i, i2, cx = _width / 2 - 1, cy = _height / 2 - 1;
...
void testTriangles() {
	int n, i, cx = _width / 2 - 1, cy = _height / 2 - 1;

	Displ_CLS(BLACK);
	n = min(cx, cy);
	for (i = 0; i < n; i += 5) {
		Displ_drawTriangle(cx, cy - i, // peak
...
void testFilledTriangles() {
	int i, cx = _width / 2 - 1, cy = _height / 2 - 1;
...
void testRoundRects() {
	int w, i, i2, red, step, cx = _width / 2 - 1, cy = _height / 2 - 1;
...
void testFilledRoundRects() {
	int i, i2, green, step, cx = _width / 2 - 1, cy = _height / 2 - 1;
...
void Displ_PerfTest() {
	...
	for (uint8_t k1 = 0; k1 < 4; k1++) {
		...
		for (uint8_t k = 1; k < 5; k++) {
			...
			Displ_CString(0, 10 + deltah + (k) * Font20.Height * 2, _width, 10 + deltah + Font20.Height * ((k) * 2 + 1),
