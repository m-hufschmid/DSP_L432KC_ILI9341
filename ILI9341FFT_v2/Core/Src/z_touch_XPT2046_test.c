...
//	uint8_t orientation;
	...
	while (!correct_orientation) {
		...
		for (uint8_t k = 0; k < 2; k++) {
			...
			Displ_CString(0, 10 + Font12.Height * k, _width, 10 + Font12.Height * (1 + k), text, Font12, 1, BLACK,
					WHITE);
		...
		for (uint8_t k = 0; k < 2; k++) {
			...
			Displ_CString(0, _height + Font12.Height * (k - 2) - 10, _width, _height + Font12.Height * (k - 1) - 10,
					text, Font12, 1, BLACK, WHITE);
		...
		if (!correct_orientation) {  // if not aligned, rotate display
			...
			Displ_CString(0, ((_height >> 1) + 11), _width, ((_height >> 1) + 20), "calibration", Font20, 1, BLUE,
					WHITE);
			HAL_Delay(2000);
			switch (orientation) {
